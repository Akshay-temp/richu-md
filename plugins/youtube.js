...
Function({
  ...
  const writer = await addAudioMetaData(await toAudio(await fs.readFileSync(media.file), 'mp4'), media.thumb, media.title, `RICHU-MD`, 'Richu Officil');
  ...
  const writer = await addAudioMetaData(await toAudio(await fs.readFileSync(media.file), 'mp4'), media.thumb, media.title, `richu-md`, 'Richu Official');
...
Function({
	...
	if (isUrl(match) && match.includes('youtu')) {
		...
		const writer = await addAudioMetaData(await toAudio(await fs.readFileSync(media.file)), thumb, media.title, `${config.BOT_INFO.split(";")[0]}`, 'Richu Official')
		return await send(message, writer, ytId[1])
		} catch {
			const response = await getJson('https://api.adithyan.ml/ytaudio?id=' + ytId[1])
			if (response.status) return await client.sendMessage(message.jid, { audio: {url: response.result }, mimetype: 'audio/mpeg', ptt: false }, { quoted: message.data })
...
Function({
	...
	if (isUrl(match) && match.includes('youtu')) {
		...
		const file = await addAudioMetaData(await fs.readFileSync(result.file), result.thumb, result.title, `${config.BOT_INFO.split(";")[0]}`, 'Richu Official')
